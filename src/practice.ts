...
const countArr = (par: string | number): number => {
...
console.log(countArr(12345));

const canYouMarry = (월소득: number, 집보유여부: boolean, 매력점수: string) => {
  let a = 월소득;
  let b: number;
  let c: number;
  if (집보유여부 === true) {
...
console.log(canYouMarry(100, true, '중'));

const cleaning = (arr: (string | number)[]): number[] => {
  let newArr: number[] = [];
  arr.forEach(item => {
    if (typeof item === 'string') {
...
const sortBack = (par: Sort): string => {
  if (typeof par.subject === 'string') {
    return par.subject;
  } else {
    return par.subject[par.subject.length - 1];
  }
};
console.log(sortBack(민수쌤));
